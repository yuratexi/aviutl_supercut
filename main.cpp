...
constexpr TCHAR* button_name[] = { "configファイルの設定方法を確認する","configファイルを開く"};
...
FILTER_DLL filter = {
    ...
    NULL,NULL,
    2,          //チェックボックスの数
    const_cast<TCHAR**>(button_name),    //チェックボックス名前郡へのポインタ(名前が入ってる配列名を書く)
    const_cast<int*>(button_default),    //初期値へのポインタ
    NULL,
    NULL,
    NULL,
    NULL,
...
void item_all_split(void* editp, FILTER* fp) {
    ...
    for (int i = 0; i < *ObjectAlloc_ptr; ++i) {
        //オブジェクトが再生位置にあったら
        if (obj->frame_begin <= now_frame && now_frame <= obj->frame_end && (int)obj->flag != 0) {
